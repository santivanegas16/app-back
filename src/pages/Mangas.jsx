...
export default function Mangas() {
    ...
    axios
      .get(
        apiUrl +
          `mangas?title=${
            store.mangas.text
          }&page=1&category=${store.mangas.checks.join(",")}`,
        Headers()
      )
      .then((res) => {
    ...
    axios
      .get(
        apiUrl +
          `mangas?title=${
            store.mangas.text
          }&page=${page}&category=${store.mangas.checks.join(",")}`,
        Headers()
      )
      .then((res) => {
        setMangas(res.data.response.mangas);
  ...
  const actionsChecks = () => {
    let checks = Object.values(inputsChecked.current)
  ...
  return (
    <main className="flex flex-col items-center min-h-screen bg-[#EBEBEB] ">
      <div
        className="w-full h-[369px] lg:h-[500px] bg-cover bg-center flex flex-col items-center justify-center"
        style={{ backgroundImage: `url(${Bg_mangas})` }}
      >
        <div className="absolute w-full h-full bg-black opacity-30 inset-0 lg:hidden"></div>
        <h1 className="z-10 w-[168px] h-[38px] font-poppins font-bold text-[40px] leading-[38.07px] m-10 text-white">
          {" "}
          Mangas{" "}
        </h1>
        <div className="z-10 flex items-center max-[380px]:w-[360px] w-[393px] lg:w-[900px] rounded-[80px] lg:rounded-[10px] bg-white">
          <span className="absolute lg:relative w-[37px] h-[37px] m-2 max-[380px]:hidden">
            {" "}
            <img
          ...
          <input
            onChange={(e) => dispatch(save_title({ title: e.target.value }))}
      ...
      <div className="bg-[#EBEBEB] lg:bg-white lg:w-[95%] lg:rounded-t-[30px] lg:rounded-b-[30px] lg:-top-[50px] relative -top-[70px] w-full rounded-t-[80px] flex flex-col items-center">
        <form
          ref={inputsChecked}
          className="flex h-[40px] w-[90%] sm:w-[50%] md:w-[40%] xl:w-[30%] justify-between mt-12"
        >
          <input
            key={0}
            className="bg-[#cacaca] cursor-pointer w-[75px] h-[35px] rounded-[50px] text-white font-poppins font-medium text-[12px] leading-[11.42px] text-center hidden lg:block hover:bg-[#999999]"
            type="button"
            value="All"
            onClick={() => dispatch(save_checks({ checks: [] }))}
          />
          {categories.map((category) => (
            <Category
              ...
              isChecked={store.mangas.checks.includes(category._id)}
            />
          ))}
        </form>
        {mangas.length !== 0 ? (
          ...
          <div className="w-full h-[244px] text-xl text-bold font-[40px] flex items-center justify-center text-center">
            Manga not found
          </div>
        )}
        <div className="flex justify-center items-center pt-5">
          {prev && (
            <button
              className="bg-gradient-to-r from-[#FF5722] to-[#F97316] text-white rounded-full m-5 w-[100px] h-[40px] transition hover:scale-110"
              value={prev}
              onClick={(e) => {
                actionNextOrPrev(e.target.value);
              }}
            >
              Prev
            </button>
